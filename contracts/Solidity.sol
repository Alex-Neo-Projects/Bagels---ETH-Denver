...
contract Solidity {
    ...
    function testFunction1(uint256 paramA) public returns (uint256) {
        return 1;
    }

    // can be called within current contract or contracts deriving from it
    function testFunction2() internal returns (uint256) {
        return 1;
    }

    // similar to internal contracts but not visible to derived contracts
    function testFunction3() private returns (uint256) {
        return 1;
    }

    // can be called from other contracts and transactions
    function testFunction4() external returns (uint256) {
        return 1;
    }

    function testFunction5() external returns (uint256 abc) {
        abc = 1;
    }

    // promises that the function won't modifiy or read the state of the blockchain
    function testFunction6() public pure returns (uint256) {
        return 1;
    }

    // view promises that the function won't modify the state of the blockchain
    function testFunction7() public view returns (uint256) {
        return 1;
    }
